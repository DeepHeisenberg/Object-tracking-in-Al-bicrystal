...
#    new_name='R5076_31053-v01.csv'   
    ...
    for i_1,j_1,i_2,j_2,i_3,j_3,i_4,j_4,k in zip(df["line_10"], df["line_11"], df["line_20"], df["line_21"],df["line_30"],df["line_31"], df["line_40"],df["line_41"], df['frame']):
        ...
        if k>kk:
            ...
            ion_num_2+=interval
            my_df=ion_sequence(file, ion_num_1, ion_num_2)
        q=my_df[my_df['my_x'].between(i_1-0.5,i_1+ 0.5, inclusive=False)]
        q=q[q['my_y'].between(j_1-0.5,j_1+ 0.5, inclusive=False)]
        if len(q)>0:
            final_df=final_df.append(q, ignore_index=True)
        print("coordinate no.1")
        q=my_df[my_df['my_x'].between(i_2-0.5,i_2+ 0.5, inclusive=False)]
        q=q[q['my_y'].between(j_2-0.5,j_2+ 0.5, inclusive=False)]
        if len(q)>0:
            final_df=final_df.append(q, ignore_index=True)    
        print("coordinate no.2")
        q=my_df[my_df['my_x'].between(i_3-0.5,i_3+ 0.5, inclusive=False)]
        q=q[q['my_y'].between(j_3-0.5,j_3+ 0.5, inclusive=False)]
        if len(q)>0:
            final_df=final_df.append(q, ignore_index=True)
        print("coordinate no.3")
        q=my_df[my_df['my_x'].between(i_4-0.5,i_4+ 0.5, inclusive=False)]
        q=q[q['my_y'].between(j_4-0.5,j_4+ 0.5, inclusive=False)]
        if len(q)>0:
            final_df=final_df.append(q, ignore_index=True)
    ...
    new_name='full_GB_3Dcoords_{}_with_m.csv'.format(name)    
