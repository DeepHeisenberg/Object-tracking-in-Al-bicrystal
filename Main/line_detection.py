...
def process_img(original_image, thresholdVal, minlinelengthVal, maxlinegapVal):
    ...
    processed_img =np.invert(cv2.cvtColor(original_image, cv2.COLOR_BGR2GRAY))# np.invert cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
...
#    video_name ='video_{}.avi'.format(X)
    ...
    thresholdVal=50 #51
#    minlinelengthVal=23 #44
#    maxlinegapVal=0.68
    minlinelengthVal=8.45
    maxlinegapVal=0.1
    ...
    o1 = (int(bbox_3[0]), int(bbox_3[1]))
    o2 = (int(bbox_3[0] + bbox_3[2]), int(bbox_3[1] + bbox_3[3]))
    #    bbox_1 = cv2.selectROI(frame, False) 
    ...
    while True:
        ...
        #        timer = cv2.getTickCount()
            ...
            bbox_30=bbox_3
            bbox_30=[int(bbox_3[0]), int(bbox_3[1]), int(bbox_3[2]), int(bbox_3[3])]
            o.append(bbox_30)
        #    print(lines)
        #    image_with_circle=Blob_detector(original_image, processed_img, minThres,maxThres,minArea)
        #    plt.figure(3)
        ...
        #                print(slope_line)      and slope_line <= 1 and slope_line>=3      
                    if (((coords[0] >= p1[0] and coords[1] >= p1[1] and coords[2] <= p2[0] and coords[3] <= p2[1]) or
                        (coords[0] >= q1[0] and coords[1] >= q1[1] and coords[2] <= q2[0] and coords[3] <= q2[1]) or
                        (coords[0] >= o1[0] and coords[1] >= o1[1] and coords[2] <= o2[0] and coords[3] <= o2[1]))and
                          slope_line<=0 and slope_line>=-5
        ...
        #            new_all_lines.append(all_lines)
            ...
            if ok:
                ...
                o1 = (int(bbox_3[0]), int(bbox_3[1]))
                o2 = (int(bbox_3[0] + bbox_3[2]), int(bbox_3[1] + bbox_3[3]))
                cv2.rectangle(frame, o1, o2, (255,255,0), 2, 1)
            else :
                # Tracking failure
...
#    X=os.path.splitext(filename)[0]
#    video_name ='video_{}.avi'.format(X)
    video_name = 'video_R5076_36585-v02.avi'
    lines = motion_tracking(video_name) 
    new_name='Jazmin_all_lines_2.csv'  
